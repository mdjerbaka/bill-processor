...
async def mark_invoice_paid(
    ...
    # Update payable if exists, or create one
    ...
    payables_svc = PayablesService(db)
    if payable:
        await payables_svc.mark_paid(payable.id)
    else:
        try:
            payable = await payables_svc.create_payable(invoice)
            await db.flush()
            await payables_svc.mark_paid(payable.id)
            logger.info(f"Created and marked paid payable for invoice {invoice_id}")
        except Exception as e:
            logger.error(f"Failed to create payable for invoice {invoice_id}: {e}")
        ...
        if await qb_svc.is_connected():
            ...
            if not invoice.qbo_bill_id:
                ...
                if bill_id:
                    ...
                    invoice.qbo_vendor_id = vendor_id
                    await db.flush()
                    logger.info(f"Sent invoice {invoice.id} to QB as bill {bill_id} (on mark-paid)")
            # Now mark the QB bill as paid
            if invoice.qbo_bill_id:
                payment_id = await qb_svc.auto_pay_bill(invoice)
                if payment_id:
                    invoice.qbo_payment_id = payment_id
                    await db.flush()
                    logger.info(f"Synced payment to QB for invoice {invoice.id}, payment {payment_id}")
                else:
                    logger.warning(
                        f"QB payment sync returned no payment ID for invoice {invoice.id} "
                        f"(qbo_bill_id={invoice.qbo_bill_id}, qbo_vendor_id={invoice.qbo_vendor_id})"
                    )
            else:
                logger.warning(f"No QBO bill ID after send attempt for invoice {invoice.id}, skipping payment")
    except Exception as e:
        logger.error(f"QB payment sync failed for invoice {invoice.id}: {e}")
